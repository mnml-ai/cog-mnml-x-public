...
class IPAdapter:
    def __init__(self, pipe, ipadapter_ckpt_path, image_encoder, device="cuda", dtype=torch.float16, resample=Image.Resampling.LANCZOS):
        self.pipe = pipe
        self.device = device
        self.dtype = dtype
        ...
        ipadapter_model_loading_time = time.time()
        ipadapter_model = torch.load(ipadapter_ckpt_path, map_location="cpu")
        print(f"Time taken to load IP adapter model: {time.time() - ipadapter_model_loading_time:.2f} seconds")
        ...
        # set image encoder
        image_encoder_loading_time = time.time()
        self.image_encoder = image_encoder
        print(f"Time taken to load image encoder model: {time.time() - image_encoder_loading_time:.2f} seconds")
    ...
    @torch.inference_mode()
    def get_prompt_embeds(self, images, negative_images=None, prompt=None, negative_prompt=None, p_embeds= None, n_embeds= None,  weight=[]):
        ...
        if prompt is not None or p_embeds is not None:
